            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">All registered NGOs</h4>
                                ...
                                <thead>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style="color: {{ $request->status=='pending' ? '#6A5ACD' :($request->status=='accepted' ? 'green' : '#DC143C') }};">{{$request->status}}</td>                                                                      
                                            <td style="width: 100px">
                                                <a href="{{url('edit'.$request->id)}}" class="btn btn-outline-secondary btn-sm edit_btn">
                                                    <i class="fas fa-pencil-alt" title="Edit"></i>
                                                </a>
                                                <a href="{{url('show'.$request->id)}}" class="btn btn-outline-secondary btn-sm delete" title="Delete">
        ...
        $(document).on('click','.edit_btn',function(){
           ...
           $.ajax({
            ...
            success:function(response){
                $('#id').val(app_id);
                $('#ngo_name').val(response.application.ngo_name);
                $('#email').val(response.application.email);
                $('#phone_number').val(response.application.phone_number);
                $('#location').val(response.application.location);
                $('#budget').val(response.application.budget);
                $('#status_box').val(response.application.status);
            }
           })
        });
